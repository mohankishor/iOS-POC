	...
	if (index <= productNumber && index >= 1)
	{
		...
		if (sqlite3_prepare_v2(mDatabaseObj, sql, -1, &statement, NULL) == SQLITE_OK) 
		{
			while (sqlite3_step(statement) == SQLITE_ROW) 
			{
				...
				prodInPage = [[FLProduct alloc] initWithTitle:title url:url price:price sku:sku imagepath:imagePath];
				[price release];
				[sku release];
				[title release];
				[priceString release];
	...
	if (index <= watchNumber  && index >= 1) 
	{
		...
		if (sqlite3_prepare_v2(mDatabaseObj, sql, -1, &statement, NULL) == SQLITE_OK) 
		{
			while (sqlite3_step(statement) == SQLITE_ROW) 
			{
				...
				watchDetails = [[FLProduct alloc] initWithTitle:nil url:nil price:nil sku:sku imagepath:imagepath];
				[sku release];
				[imagepath release];
			}
		}
